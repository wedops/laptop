...
Set up a fresh Mac for [wedops](https://github.com/wedops) development in one idempotent pass — inspired by [thoughtbot's `laptop`](https://github.com/thoughtbot/laptop).
...
- Installs the **Xcode Command Line Tools** and **Homebrew** if missing
...
The `Brewfile` is the source of truth for what a machine needs. Add a line, re-run `./mac`, done. Keep each entry's trailing comment honest about why it's there.
